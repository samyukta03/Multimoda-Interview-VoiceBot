...
def run_code():
    xx = pd.read_excel('D:/Prediction-of-Job-Interview-Performance-by-extracting-Lexical-features-/Top_seven_features.xlsx',sheet_name = 'Excited')
    ...
    ws = wb['Coeff']
    ws.cell(row=2,column=1,value='Excited')
    ws.cell(row=2,column=2,value=correlation[0,1])
    wb.save('coeff.xlsx')
    ...
    ws=wb['Final']
    ws.cell(row=2,column=3,value=(y_pred[0]*10))

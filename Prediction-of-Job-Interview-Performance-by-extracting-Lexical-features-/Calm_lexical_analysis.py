...
def run_code():
    xx = pd.read_excel('D:/Prediction-of-Job-Interview-Performance-by-extracting-Lexical-features-/Top_seven_features.xlsx',sheet_name = 'Calm')
    ...
    ws = wb['Coeff']
    ws.cell(row=4,column=1,value='Calm')
    ws.cell(row=4,column=2,value=correlation[0,1])
    wb.save('coeff.xlsx')
    ...
    ws=wb['Final']
    ws.cell(row=2,column=2,value=(y_pred[0]*10))

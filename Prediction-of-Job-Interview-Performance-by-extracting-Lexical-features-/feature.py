...
import nltk
nltk.download('all')
ws2 = openpyxl.load_workbook('Interview_transcripts.xlsx')
...
for i in range(0,69):
    ...
    for j in range(2,x+1,2):
        if (sheetx.cell(row=j, column=2)).value is not None :
...
import openpyxl
ws1 = openpyxl.load_workbook('Interview_transcripts.xlsx')
sheet2 = []
sheet2 = ws1.sheetnames
# for i in range(0,2):

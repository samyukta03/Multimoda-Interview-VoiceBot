...
def run_code():
    xx = pd.read_excel('D:/Prediction-of-Job-Interview-Performance-by-extracting-Lexical-features-/Top_seven_features.xlsx',sheet_name = 'StructuredAnswers')
    ...
    ws = wb['Coeff']
    ws.cell(row=9,column=1,value='StructuredAnswers')
    ws.cell(row=9,column=2,value=correlation[0,1])
    wb.save('coeff.xlsx')
    ...
    ws=wb['Final']
    ws.cell(row=2,column=9,value=(y_pred[0]*10))
